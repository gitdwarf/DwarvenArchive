...
class DwarvenArchiveApp(Gtk.Application):
    ...
    def _on_open(self, app, files, n_files, hint):
        ...
        paths = [gf.get_path() for gf in files if gf.get_path()]
        if paths:
    ...
    def _on_activate(self, app):
        ...
        if not args:
            InfoWindow(self, self._on_paths_dropped).present()
            return

        # Resolve all args to absolute paths.
        paths = []
        for arg in args:
            if arg == '-':
                paths.append(arg)
            else:
                p = arg if arg.startswith('/') else os.path.abspath(arg)
    ...
    def _on_paths_dropped(self, paths):
        """Called by InfoWindow when files are dropped onto it."""
        self._open_paths(paths)

    def _open_paths(self, paths):
        """Create and present an ArchiveWindow for one or more paths."""
        if len(paths) == 1:
            path = paths[0]
            data = formats.DirData(path) if os.path.isdir(path) else formats.FileData(path)
        else:
            data = formats.MultiData(paths)
