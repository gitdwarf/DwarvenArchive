...
class PipeThroughCommand:
    ...
    def __init__(self, command, src=None, dst=None, acceptable_returncodes=()):
        ...
        self.acceptable_returncodes = acceptable_returncodes
    ...
    def wait(self):
        ...
        if self.error:
            raise self.error
        if returncode != 0 and returncode not in self.acceptable_returncodes:
            err = subprocess.CalledProcessError(returncode, self.command)
            err.stderr = stderr_data
            raise err
        return returncode
...
def pipe_through_command(command, src, dst, acceptable_returncodes=()):
    ...
    try:
        ...
        pass
    current_command = PipeThroughCommand(command, src, dst, acceptable_returncodes)
    try:
...
class Operation:
    ...
    def can_handle(self, data):
        return self.is_available() and isinstance(data, FileData)

    def save_to_stream(self, data, stream):
        pipe_through_command(self.command, data.source, stream)

    def is_available(self):
        """Return True if the external tool required by this op is installed."""
...
class Decompress(Operation):
    ...
    def can_handle(self, data):
        ...
        if not isinstance(data, FileData):
            return False
        if data.default is self:
            return True
        return data.path.lower().endswith('.' + self.extension)

class Extract(Operation):
    """Extract an archive to a directory."""
    type = 'inode/directory'

    def __init__(self, extension, command, supports_password=False, acceptable_returncodes=()):
    ...
    def can_handle(self, data):
        ...
        if not isinstance(data, FileData):
            return False
        # Show extraction only if magic bytes identified this format, or
        # the filename ends with this extension (covers alias extensions).
        if data.default is self:
            return True
        path_lower = data.path.lower()
        return path_lower.endswith('.' + self.extension)
    ...
    def save_to_file(self, data, path, password=None):
        ...
        if command.find('%s') != -1:
            ...
            source = None
        try:
            pipe_through_command(command, source, None, self.acceptable_returncodes)
        finally:
            try:
                os.rmdir(path)
            except Exception:
...
class Archive(Operation):
    ...
    def can_handle(self, data):
        return self.is_available() and isinstance(data, (DirData, FileData, MultiData))

    def save_to_stream(self, data, stream, password=None):
        if isinstance(data, MultiData):
            # chdir to common parent so relative paths are used inside archive.
            common = os.path.commonpath([os.path.dirname(p.rstrip('/')) for p in data.paths])
            os.chdir(common)
            rel_paths = ' '.join(
                shell_escape(os.path.relpath(p, common)) for p in data.paths)
            cmd = self.command
            # zip/jar -r flag causes "nothing to do" for files-only lists;
            # drop it when no directories are present.
            has_dirs = any(os.path.isdir(p) for p in data.paths)
            if not has_dirs and self.extension in ('zip', 'jar'):
                cmd = cmd.replace('zip -9qr', 'zip -9q', 1)
            command = cmd % rel_paths
        else:
            os.chdir(os.path.dirname(data.path.rstrip('/')))
...
class FileArchive(Operation):
    ...
    def save_to_stream(self, data, stream, password=None):
        ...
        try:
            # basename is pre-escaped for MultiData; single path needs escaping.
            src = basename if isinstance(data, MultiData) else shell_escape(basename)
            command = self.command % (shell_escape(tmp_path), src)
...
# unzip exit codes: 0=ok, 1=warning (non-fatal), 2=generic warning (absolute
# paths stripped, charset conversion failures, etc.) -- files still extracted.
# unrar exit codes: 0=ok, 1=warning (non-fatal).
zip   = Extract('zip',   'unzip -q %s', supports_password=True,  acceptable_returncodes=(1, 2))
...
rar   = Extract('rar',   'unrar x %s',                           acceptable_returncodes=(1,))
...
jar   = Extract('jar',   'unzip -q %s', supports_password=True,  acceptable_returncodes=(1, 2))
...
def first_available_archive():
    """Return the first Archive/FileArchive op in operations order whose
    tool is installed on this system. Falls back to make_archive (zip)."""
    for op in operations:
        if isinstance(op, (Archive, FileArchive)) and op.is_available():
            return op
    return make_archive
...
class FileData:
    ...
    def __init__(self, path):
        ...
        # For plain files (not detected as an archive or compressed stream),
        # default to the first available archive format in list order.
        if isinstance(self.default, Compress):
            self.default = first_available_archive()
        ...
        if path == '-':
            ...
            for ext in known_extensions:
                if path.endswith('.' + ext):
                    ...
                    if len(new) < len(name):
                        name = new
            # For archive default on a plain file (no known ext stripped),
            # strip any remaining extension so 'file.png' -> 'file.zip'
            # not 'file.png.zip'.
            if isinstance(self.default, (Archive, FileArchive)) and name == path:
    ...
    def check_if_encrypted(self):
        ...
        try:
            if ext.endswith('.zip') or ext.endswith('.jar'):
                # unzip -Z -v lists each entry with flags; encrypted entries
                # have 'B' (or other flag chars) in the method field.
                # Use zipinfo with verbose output and check for the specific
                # encryption flag character, not just the word "encrypted".
                result = subprocess.run(
                    ['unzip', '-Z', '-v', self.path],
                    capture_output=True, text=True, timeout=5)
                for line in result.stdout.split('\n'):
                    # zipinfo -v output has 'Encr:' field per file
                    if 'encr:' in line.lower() and 'yes' in line.lower():
                        return True
        except Exception:
            pass
...
class MultiData:
    """Multiple files/folders to be archived together.

    Always an archive operation -- extraction makes no sense for multiple items.
    The default name is derived from the first path.
    """
    mode = None

    def __init__(self, paths):
        self.paths = paths
        self.path = paths[0]   # primary path -- used for chdir and naming
        self.default = first_available_archive()
        base = os.path.basename(self.path.rstrip('/'))
        if '.' in base:
            base = base[:base.rindex('.')]
        self.default_name = os.path.join(
            os.path.dirname(self.path), base + '.' + self.default.extension
        )
