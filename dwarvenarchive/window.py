...
class ArchiveWindow(Gtk.ApplicationWindow):
    ...
    def _build_ui(self):
        ...
        self.delete_choice = Gtk.CheckButton(label=_('Delete source afterwards?'))
    ...
    def set_data(self, data):
        ...
        if isinstance(data, (formats.DirData, formats.MultiData)):
        ...
        # Delete only makes sense for a single source file.
        if isinstance(data, formats.DirData):
            self.delete_choice.set_visible(True)
        elif isinstance(data, formats.FileData) and data.path != '-':
            self.delete_choice.set_visible(True)
        elif isinstance(data, formats.MultiData):
            self.delete_choice.set_label(_('Delete sources afterwards?'))
            self.delete_choice.set_visible(True)
    ...
    def _on_op_changed(self, combo):
        ...
        if not op:
            return
        if isinstance(self.data, (formats.DirData, formats.FileData, formats.MultiData)) and op.supports_password:
    ...
    def _resolve_path(self, name, op):
        ...
        if op.type == 'inode/directory':
            # Use name directly as the extraction target directory.
            # Do NOT use a subdirectory if name already exists as a dir --
            # that was causing silent extraction into a subdir of the dest.
            return name
        return name
    ...
    def _run_save(self, path):
        try:
            ...
            self._show_error(self._format_error(e))
    ...
    def _do_save(self, path):
        ...
        self.op_combo.set_sensitive(False)
        self.password_choice.set_sensitive(False)
        ...
        self.op_combo.set_sensitive(True)
        # Re-evaluate password sensitivity based on current op.
        self._on_op_changed(self.op_combo)
        ...
        if self.delete_choice.get_active():
            paths = self.data.paths if isinstance(self.data, formats.MultiData) else [self.data.path]
            for p in paths:
                try:
                    if os.path.isdir(p):
                        import shutil
    ...
    def _estimate_source_size(self):
        ...
        if isinstance(self.data, formats.DirData):
            try:
                ...
                pass
        elif isinstance(self.data, formats.FileData):
            try:
                return os.path.getsize(self.data.path)
            except Exception:
                pass
        elif isinstance(self.data, formats.MultiData):
            try:
                total = 0
                for p in self.data.paths:
                    if os.path.isdir(p):
                        import subprocess
                        r = subprocess.run(['du', '-sb', p],
                                           capture_output=True, text=True, timeout=5)
                        if r.returncode == 0:
    ...
    def _wait_for_save(self, path, source_size):
        ...
        while self.save_thread.is_alive():
            # In GTK4 the frame clock drives redraws -- we must let the main
            # loop iterate freely (including blocking on the next event) to
            # allow frame clock ticks to fire and the pulse bar to repaint.
            ctx.iteration(True)   # block until next event (frame tick etc)
            while ctx.pending():
                ctx.iteration(False)
            now = time.time()
    ...
    def _update_progress(self, path, source_size):
        """Update progress bar -- pulse (Cylon/KITT scanner) with % text."""
        pct_text = None
        if os.path.exists(path) and source_size > 0:
            try:
                if os.path.isdir(path):
                    import subprocess
                    r = subprocess.run(['du', '-sb', path],
                                       capture_output=True, timeout=1)
                    cur = int(r.stdout.split()[0]) if r.returncode == 0 else 0
                else:
                ...
                pass
        # Always pulse for the scanner animation; show % if available
        self.progress_bar.pulse()
        self.progress_bar.set_text(pct_text if pct_text else _('Working...'))
    ...
    def _dest_exists_dialog(self, path):
        """Inform user that extraction destination already exists."""
        from gi.repository import Gdk
        dialog, vbox = _make_dialog(self, _('Destination Exists!'))
        vbox.append(_make_icon_row(
            'dialog-warning',
            f"<b>{os.path.basename(path)}</b> already exists!\n\n"
            f"Return and choose a different destination.",
            markup=True))
        return_btn = Gtk.Button(label=_('Return'))
        return_btn.connect('clicked', lambda b: dialog.close())
        vbox.append(_make_button_row(return_btn))
    ...
    def _confirm_overwrite(self, path):
        ...
        if os.path.isdir(path):
            msg = f'<b>{os.path.basename(path)}</b> already exists.\nExtract into it anyway?'
        else:
            msg = f'<b>{os.path.basename(path)}</b> already exists.\nOverwrite it?'
    ...
    def _format_error(self, e):
        """Return a human-readable error string from an exception."""
        import subprocess
        if isinstance(e, subprocess.CalledProcessError):
            cmd = e.cmd.split()[0] if isinstance(e.cmd, str) else str(e.cmd)
            msg = _('{cmd} failed (exit {code})').format(
                cmd=cmd, code=e.returncode)
            if e.stderr:
                detail = e.stderr.decode('utf-8', errors='replace').strip() if isinstance(e.stderr, bytes) else str(e.stderr).strip()
                if detail:
                    msg += '\n\n' + detail
            return msg
        return str(e)
